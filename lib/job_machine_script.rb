...
module JobMachineScript
  ...
  class TestSuiteCommand
    ...
    def docker_compose_command
      "docker-compose -f .saturnci/docker-compose.yml run -e #{docker_compose_env_vars} saturn_test_app #{rspec_command}"
    end
    ...
    def docker_compose_env_vars
      @docker_compose_env_vars.map { |key, value| "#{key}=#{value}" }.join(" ")
    end
